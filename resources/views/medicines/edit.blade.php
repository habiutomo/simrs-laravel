...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('medicines.update', $item->id) }}">
            @csrf @method('PUT')
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="medicine_category_id" class="form-select @error('medicine_category_id') is-invalid @enderror">
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ old('medicine_category_id', $item->medicine_category_id)==$cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="code" class="form-control @error('code') is-invalid @enderror" value="{{ old('code', $item->code) }}" readonly>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="generic_name" class="form-control @error('generic_name') is-invalid @enderror" value="{{ old('generic_name', $item->generic_name) }}">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select name="unit" class="form-select @error('unit') is-invalid @enderror">
                        <option value="">-- Pilih --</option>
                        <option value="tablet" {{ old('unit', $item->unit)=='tablet' ? 'selected' : '' }}>Tablet</option>
                        <option value="kapsul" {{ old('unit', $item->unit)=='kapsul' ? 'selected' : '' }}>Kapsul</option>
                        <option value="botol" {{ old('unit', $item->unit)=='botol' ? 'selected' : '' }}>Botol</option>
                        <option value="vial" {{ old('unit', $item->unit)=='vial' ? 'selected' : '' }}>Vial</option>
                        <option value="ampul" {{ old('unit', $item->unit)=='ampul' ? 'selected' : '' }}>Ampul</option>
                        <option value="sirup" {{ old('unit', $item->unit)=='sirup' ? 'selected' : '' }}>Sirup</option>
                        <option value="salep" {{ old('unit', $item->unit)=='salep' ? 'selected' : '' }}>Salep</option>
                        <option value="tetes" {{ old('unit', $item->unit)=='tetes' ? 'selected' : '' }}>Tetes</option>
                    </select>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" name="min_stock" class="form-control @error('min_stock') is-invalid @enderror" value="{{ old('min_stock', $item->min_stock) }}">
                    @error('min_stock') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">Tanggal Kedaluwarsa</label>
                    <input type="date" name="expired_date" class="form-control @error('expired_date') is-invalid @enderror" value="{{ old('expired_date', $item->expired_date) }}">
                    @error('expired_date') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">Produsen</label>
                    <input type="text" name="manufacturer" class="form-control @error('manufacturer') is-invalid @enderror" value="{{ old('manufacturer', $item->manufacturer) }}">
