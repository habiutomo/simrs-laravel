...
<div class="page-header">
    <h5 class="mb-0">Tambah Rawat Inap</h5>
...
<div class="card">
    <div class="card-body">
        @php
            ...
            $registrations = \App\Models\Registration::where('type', 'rawat_inap')->where('status', 'waiting')->get();
        @endphp
        <form method="POST" action="{{ route('inpatient-admissions.store') }}">
            @csrf
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" name="admission_date" class="form-control @error('admission_date') is-invalid @enderror" value="{{ old('admission_date') }}" required>
                    @error('admission_date')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Jam Masuk</label>
                    <input type="time" name="admission_time" class="form-control @error('admission_time') is-invalid @enderror" value="{{ old('admission_time') }}">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                        ...
                        <option value="discharged" {{ old('status')=='discharged' ? 'selected' : '' }}>Pulang</option>
                        <option value="transferred" {{ old('status')=='transferred' ? 'selected' : '' }}>Dipindah</option>
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <textarea name="primary_diagnosis" class="form-control @error('primary_diagnosis') is-invalid @enderror" rows="2">{{ old('primary_diagnosis') }}</textarea>
