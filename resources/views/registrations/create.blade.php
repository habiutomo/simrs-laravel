            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="insurance_id" class="form-select @error('insurance_id') is-invalid @enderror">
                        ...
                        <option value="{{ $ins->id }}" {{ old('insurance_id')==$ins->id ? 'selected' : '' }}>{{ $ins->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="type" class="form-select @error('type') is-invalid @enderror" required>
                        ...
                        <option value="rawat_jalan" {{ old('type')=='rawat_jalan' ? 'selected' : '' }}>Rawat Jalan</option>
                        <option value="igd" {{ old('type')=='igd' ? 'selected' : '' }}>IGD</option>
                        <option value="rawat_inap" {{ old('type')=='rawat_inap' ? 'selected' : '' }}>Rawat Inap</option>
                    </select>
                    @error('type')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Waktu Pendaftaran</label>
                    <input type="datetime-local" name="registration_time" class="form-control @error('registration_time') is-invalid @enderror" value="{{ old('registration_time') }}" required>
                    @error('registration_time')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Rujukan Dari</label>
                    <input type="text" name="referral_from" class="form-control @error('referral_from') is-invalid @enderror" value="{{ old('referral_from') }}">
                    @error('referral_from')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-12 mb-3">
                    <label class="form-label">Keluhan</label>
                    <textarea name="complaint" class="form-control @error('complaint') is-invalid @enderror" rows="3">{{ old('complaint') }}</textarea>
