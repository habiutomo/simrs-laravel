...
<div class="page-header">
    ...
    <a href="{{ route('radiology-requests.create') }}" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i> Tambah</a>
</div>
<div class="card">
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                <thead><tr><th>No Rad</th><th>Pasien</th><th>Pemeriksaan</th><th>Prioritas</th><th>Status</th><th>Aksi</th></tr></thead>
                <tbody>
                @forelse($items as $item)
                <tr>
                    <td>{{ $item->request_number }}</td>
                    <td>{{ $item->patient->name ?? '-' }}</td>
                    <td>{{ $item->radiologyTest->name ?? '-' }}</td>
                    ...
                    <td>
                        <a href="{{ route('radiology-requests.show', $item->id) }}" class="btn btn-sm btn-info text-white"><i class="fas fa-eye"></i></a>
                        <a href="{{ route('radiology-requests.edit', $item->id) }}" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                        <form method="POST" action="{{ route('radiology-requests.destroy', $item->id) }}" class="d-inline" onsubmit="return confirm('Hapus data?')">@csrf @method('DELETE')<button class="btn btn-sm btn-danger"><i class="fas fa-trash"></i></button></form>
                    </td>
                </tr>
                @empty
                <tr><td colspan="6" class="text-center text-muted py-4">Tidak ada data permintaan radiologi</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
