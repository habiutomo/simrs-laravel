@extends('layouts.app')
@section('title', 'Tambah Permintaan Radiologi')
@section('content')
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="radiology_test_id" class="form-select @error('radiology_test_id') is-invalid @enderror" required>
                        ...
                        <option value="{{ $test->id }}" {{ old('radiology_test_id')==$test->id ? 'selected' : '' }}>{{ $test->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="priority" class="form-select @error('priority') is-invalid @enderror" required>
                        <option value="routine" {{ old('priority')=='routine' ? 'selected' : '' }}>Rutin</option>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="datetime-local" name="requested_at" class="form-control @error('requested_at') is-invalid @enderror" value="{{ old('requested_at') }}" required>
                    @error('requested_at')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-12 mb-3">
                    <label class="form-label">Informasi Klinis</label>
                    <textarea name="clinical_info" class="form-control @error('clinical_info') is-invalid @enderror" rows="3">{{ old('clinical_info') }}</textarea>
