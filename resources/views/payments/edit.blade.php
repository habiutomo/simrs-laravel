...
<div class="card">
    <div class="card-body">
        @php
            $unpaidBills = \App\Models\PatientBill::all();
        @endphp
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="patient_bill_id" class="form-select @error('patient_bill_id') is-invalid @enderror" id="patientBillSelect" required>
                        ...
                        <option value="{{ $bill->id }}" data-patient="{{ $bill->patient_id }}" {{ old('patient_bill_id', $item->patient_bill_id)==$bill->id ? 'selected' : '' }}>{{ $bill->bill_number }} - {{ $bill->patient->name ?? '' }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" name="amount" class="form-control @error('amount') is-invalid @enderror" value="{{ old('amount', $item->amount) }}" min="0" required>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="method" class="form-select @error('method') is-invalid @enderror" required>
                        ...
                        <option value="cash" {{ old('method', $item->method)=='cash' ? 'selected' : '' }}>Tunai</option>
                        <option value="transfer" {{ old('method', $item->method)=='transfer' ? 'selected' : '' }}>Transfer</option>
                        <option value="debit" {{ old('method', $item->method)=='debit' ? 'selected' : '' }}>Debit</option>
                        <option value="credit" {{ old('method', $item->method)=='credit' ? 'selected' : '' }}>Kredit</option>
                        <option value="insurance" {{ old('method', $item->method)=='insurance' ? 'selected' : '' }}>Asuransi</option>
                        <option value="other" {{ old('method', $item->method)=='other' ? 'selected' : '' }}>Lainnya</option>
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                        ...
                        <option value="confirmed" {{ old('status', $item->status)=='confirmed' ? 'selected' : '' }}>Dikonfirmasi</option>
                    </select>
                    @error('status')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Tanggal Bayar</label>
                    <input type="datetime-local" name="payment_date" class="form-control @error('payment_date') is-invalid @enderror" value="{{ old('payment_date', $item->payment_date ? $item->payment_date->format('Y-m-d\TH:i') : '') }}" required>
                    @error('payment_date')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Diterima Oleh</label>
                    <input type="text" name="received_by" class="form-control @error('received_by') is-invalid @enderror" value="{{ old('received_by', $item->received_by) }}">
